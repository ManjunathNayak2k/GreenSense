...
def longestCommonSubsequence_brute(text1, text2):
    ...
    def calculateLCS(text1, text2, m, n, memo):
        if m == 0 or n == 0:
            return 0
        ...
        if text1[m-1] == text2[n-1]:
            ...
            memo[m][n] = 1 + calculateLCS(text1, text2, m-1, n-1, memo)
        else:
            # If the last characters of both texts are different, calculate the LCS length by considering both possibilities
            memo[m][n] = max(calculateLCS(text1, text2, m-1, n, memo), calculateLCS(text1, text2, m, n-1, memo))
        
        return memo[m][n]
    
    return calculateLCS(text1, text2, len(text1), len(text2), memo)
