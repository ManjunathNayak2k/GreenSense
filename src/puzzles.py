...
def longestCommonSubsequence_brute(text1, text2):
    ...
    def calculateLCS(text1, text2, memo):
        ...
        if text1[-1] == text2[-1]:
            # If the last characters of both texts are the same, add 1 to the LCS length and recursively calculate the LCS length for the remaining texts
            memo[len(text1)][len(text2)] = 1 + calculateLCS(text1[:-1], text2[:-1], memo)
        else:
            # If the last characters of both texts are different, recursively calculate the LCS length for the remaining texts and take the maximum
            memo[len(text1)][len(text2)] = max(calculateLCS(text1[:-1], text2, memo), calculateLCS(text1, text2[:-1], memo))
